...
void Motor::set_position(float new_position) 
{
    if (in_range(new_position))
    {
      ...
      u_int16_t maxSpeed = 50;

      char send_buf[9] = {0};

      memmove(send_buf,zero_pos_msg,8);

      send_buf[0] = { zero_pos_msg[0] };
      send_buf[1] = { 0x00 };
      ...
      CAN_BUS_Send(get_position_msg, this->can_address_);

      usleep(readDelay);
    
      can_reply = CAN_BUS_Receive(this->can_address_, get_position_msg[0]);
...

      {
          ...
          if ( rec_buf[1] == get_position_msg[0] &&  rec_buf[0] == (0X0F & this->can_address_) )
          {
              this->position_ =  (rec_buf[5] + ( rec_buf[6] << 8 ) + ( rec_buf[7] << 16 ) + (rec_buf[8] << 24)) * 0.01;
          }
          else printf("NOPOS\n");
      }
